  ...
  return class {
    ...
    save() {
      if (this._ephemeral === true) {
        // TODO: should this apply id and modify ephemeral?
        return db.insert(this.data).into(this._entityName()).returning('id')
          .then((result) => result[0]);
      } else {
        if (this.data.id == null) throw new Error("trying to update a record that does not exist!");
        return db.update(this.data).into(this._entityName()).where({ id: this.data.id })
    ...
    static getById(id) {
      return db.select('*').from(this._entityName()).where({ id })
        .then((rows) => rows.map((row) => new this(row, false)));
    }
    static getCount(condition = {}) {
      return db.count('*').from(this._entityName()).where(condition)
