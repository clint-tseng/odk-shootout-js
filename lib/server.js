...
const { submissionsToCsvStream } = require('./xml');
...
// accepts optional templateSubmissionID qs param indicating the record whose schema to
// follow on export.
service.get('/submission/:formId.csv', (request, response) => {
  const go = (template) => {
    Submission.queryByFormId(request.params.formId).stream((stream) => {
      submissionsToCsvStream(stream, template).pipe(response);
    });
  }

  if (request.query.templateSubmissionId != null)
    Submission.getSingle(request.params.formId, request.query.templateSubmissionId).then((template) => {
      if (template == null)
        notfound(response);
      else
        go(template.xml);
